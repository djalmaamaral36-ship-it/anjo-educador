...
export default function PaxStudentCard({
  ...
  return (
    ...
    {/* BARRA DE ATALHOS RÁPIDOS DA ROTINA (Cores Vivas, Contrastantes & Intuitivas) */}
      <div className="bg-slate-100/90 border border-slate-200 rounded-2xl p-2.5 sm:p-3 shadow-xs">
        <div className="flex items-center gap-2.5 overflow-x-auto pb-0.5 scrollbar-thin text-xs">
          <span className="text-[11px] font-black uppercase text-slate-700 tracking-wider shrink-0 flex items-center gap-1.5 pl-1">
            <span className="text-amber-500 text-sm">⚡</span>
            <span>Ir para:</span>
          </span>

          {/* 🍼 Alimentação - Âmbar / Dourado Quente */}
          <button
            type="button"
            onClick={() => document.getElementById('secao-alimentacao')?.scrollIntoView({ behavior: 'smooth' })}
            className="px-3.5 py-2 rounded-xl bg-amber-50 hover:bg-amber-100 border-2 border-amber-300 hover:border-amber-400 text-amber-950 font-black shrink-0 transition flex items-center gap-1.5 cursor-pointer shadow-2xs hover:shadow-xs active:scale-95"
            title="Ir direto para Alimentação (Água, Mamadeiras e Sucos)"
          >
            <span>🍼 Alimentação</span>
          </button>

          {/* 🍛 Refeições - Laranja Coral Vibrante */}
          <button
            type="button"
            onClick={() => document.getElementById('secao-refeicoes')?.scrollIntoView({ behavior: 'smooth' })}
            className="px-3.5 py-2 rounded-xl bg-orange-50 hover:bg-orange-100 border-2 border-orange-300 hover:border-orange-400 text-orange-950 font-black shrink-0 transition flex items-center gap-1.5 cursor-pointer shadow-2xs hover:shadow-xs active:scale-95"
            title="Ir direto para Refeições Sólidas (Café, Almoço, Lanche, Jantar)"
          >
            <span>🍛 Refeições</span>
          </button>

          {/* 💤 Soneca - Roxo Lilás Acolhedor */}
          <button
            type="button"
            onClick={() => document.getElementById('secao-soneca')?.scrollIntoView({ behavior: 'smooth' })}
            className="px-3.5 py-2 rounded-xl bg-purple-50 hover:bg-purple-100 border-2 border-purple-300 hover:border-purple-400 text-purple-950 font-black shrink-0 transition flex items-center gap-1.5 cursor-pointer shadow-2xs hover:shadow-xs active:scale-95"
            title="Ir direto para Soneca e Descanso"
          >
            <span>💤 Soneca</span>
          </button>

          {/* 🧷 Fraldas & Saúde - Verde Esmeralda */}
          <button
            type="button"
            onClick={() => document.getElementById('secao-saude')?.scrollIntoView({ behavior: 'smooth' })}
            className="px-3.5 py-2 rounded-xl bg-emerald-50 hover:bg-emerald-100 border-2 border-emerald-300 hover:border-emerald-400 text-emerald-950 font-black shrink-0 transition flex items-center gap-1.5 cursor-pointer shadow-2xs hover:shadow-xs active:scale-95"
            title="Ir direto para Fraldas, Evacuação, Febre e Saúde"
          >
            <span>🧷 Fraldas & Saúde</span>
          </button>

          {/* 🎨 Agenda de Atividades - Degradê Verde Esmeralda / Teal */}
          <button
            type="button"
            onClick={() => document.getElementById('agenda-atividades-section')?.scrollIntoView({ behavior: 'smooth' })}
            className="px-4 py-2 rounded-xl bg-gradient-to-r from-emerald-600 via-teal-600 to-teal-700 hover:from-emerald-700 hover:to-teal-800 text-white font-black shrink-0 transition flex items-center gap-2 cursor-pointer shadow-md hover:shadow-lg active:scale-95 border border-emerald-500"
            title="Ir direto para a Agenda de Atividades Pedagógicas"
          >
            <span>🎨 Agenda de Atividades</span>
            <span className="bg-black/25 text-[10px] px-2 py-0.5 rounded-md uppercase tracking-wider font-extrabold text-emerald-100">
              Semanal • 07h às 17h
            </span>
          </button>
        </div>
