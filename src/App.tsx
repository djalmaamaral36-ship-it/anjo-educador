...
import { initializeDB, getFromDB, saveToDB, SALAS_INICIAIS } from './data';
...
const generateAuraJwtAsync = async (payload: any, secret: string): Promise<string> => {
  ...
  const getAuraTokenAsync = async (): Promise<string> => {
    ...
    // Incluir parâmetros no nível raiz para que a API da Aura monte o redirect "next" com query params,
  ...
  return (
    <div className={`min-h-screen max-w-full ${accessibility.darkMode ? 'bg-[#0f172a] text-slate-100 dark-mode' : 'bg-cozy-cream text-slate-800'} flex flex-col font-sans transition-all selection:bg-serene-blue selection:text-white ${getFontSizeClass()}`}>
      ...
      {/* MODAL DE SEGURANÇA CONTRA ACESSO DE TERCEIROS DENTRO DA SIMULAÇÃO */}
      {showPinModal && (
              ...
              >
                <div className="space-y-3">
                  ...
                  <p className="text-[11px] text-slate-500 leading-relaxed font-medium">
                    Monitoramento de idosos, controle de remédios com alarmes, registro de sinais vitais, agenda médica e alertas instantâneos de emergência no WhatsApp.
                  </p>
                </div>
